...
The language-neutral `csmi.value-transfer` profile at
`profiles/value-transfer/0.1/profile.md` adds identity-separating copy, move,
conversion, boxing, and unboxing meaning to core transfers and defines exact
type-value and implicit-operation facts. The C and C++ profile family at
`profiles/cpp/0.1/profile.md` uses `csmi.c-cpp-resolution` for shared resolver
and artifact applicability and `csmi.cpp` for C++ declaration facts. These
profiles are independently versioned `0.1.0` vocabularies used with core
`semanticModelVersion: "0.1"` and `serializationVersion: "0.1-json"`; neither
changes the meaning or shape of the core versions. A consumer MUST support each
...
The analyzer-neutral `csmi.conditional-type-refinement` `0.1.0` profile at
`profiles/conditional-type-refinement/0.1/profile.md` binds a predicate result
to an exact callable, zero-based parameter ordinal, and structured target type.
It distinguishes biconditional true-and-false refinement from positive-only
true-branch refinement without adding source-language guard forms to core.
Unsupported targets, ambiguous or conflicting overload semantics, and
uninterpretable required use remain fail-closed outcomes rather than empty
facts.
...
The separately versioned normative Rust profile is defined at
`profiles/rust/0.1/profile.md`. It uses the core profile mechanics in section
3.6 and does not add Rust-specific fields or enum values to this core.

The separately versioned C and C++ profile family is defined at
`profiles/cpp/0.1/profile.md`. Its `csmi.c-cpp-resolution` vocabulary covers
shared translation-unit applicability, while its C++-only
`csmi.cpp.declaration` keys require exact
artifact scope and structured resolver evidence; matching namespace, type, or
rendered signature text is never a fallback. C++ value-transfer meaning is
defined independently by `profiles/value-transfer/0.1/profile.md`, so consumers
can implement the analyzer-neutral transfer vocabulary without implementing
C++ identity.
...
CSMI 0.1 also assigns `csmi.conditional-type-refinement` `0.1.0` in the
[conditional type-refinement profile](https://csmi.brokk.ai/profiles/conditional-type-refinement/).
It defines branch-sensitive narrowing for an exact callable parameter and exact
structured target type. Biconditional semantics include false-branch exclusion;
